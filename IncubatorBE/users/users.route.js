...
router.post('/', (req, res, next)=>{
  ...
  // req.getValidationResult().then(function(result) {
    ...
    securityManager.createUser(userObj).then((message)=>{
      ...
      userObj._id = message.data.id;
      userObj._rev = message.data.rev;
...
router.patch('/',auth, (req, res, next)=>{
  ...
  switch (operType) {
    case 'assign-mentor':
      ...
      break;
    case 'assgin-role':
      securityManager.assignRole(userObj).then((userMsg)=>{
        renderResponseUtil.sendResponse(req, res, userMsg);
      }, (errorMessage)=>{
