...
exports.insertAttachment = function(docObject, file, attached){
...
exports.removeAttachement = function(id, rev){
    ...
    return new Promise((resolve, reject)=>{
      // ModelUtil.findById(_id).then((document)=>{
        ...
        // }
        ModelUtil.findById(id).then((freshAttach)=>{
          ModelUtil.deleteDoc(freshAttach._id, freshAttach._rev).then((result)=>{
...
exports.removeAttachements = function(attachments){
    ...
    return new Promise((resolve, reject)=>{
      ...
      for (var i = 0; i < attachments.length; i++) {
        promises.push(ModelUtil.findById(attachments[i].id));
      }
      Promise.all(promises).then((freshAttachs)=>{
        promises = [];
        for (var i = 0; i < freshAttachs.length; i++) {
          promises.push(ModelUtil.deleteDoc(freshAttachs[i]._id, freshAttachs[i]._rev));
        }
  ...
  return new Promise((resolve, reject)=>{
      ...
      fs.readFile(file.path, function(err, data) {
        if (!err && file) {
          ...
          db.attachment.insert(id, name, data, file.type, {
              rev: rev
...
exports.attachAttachments = function(attachments, isJson){

  return new Promise((resolve, reject)=>{
    let promises = [];
    for (var i = 0; i < attachments.length; i++) {
      promises.push(ModelUtil.findById(isJson?attachments[i].id : attachments[i]));
    }
    let existingDocs = [];
    Promise.all(promises).then((attachs)=>{
      for (var i = 0; i < attachs.length; i++) {
        if(attachs[i]){
          attachs[i].attached = true;
          existingDocs.push(attachs[i]);
        }
      }
      ModelUtil.bulkUpdates(existingDocs).then(()=>{
        resolve();
      }, (err)=>{
        reject(err)
      });
