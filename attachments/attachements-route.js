...
router.post('/',multipartMiddleware, (req, res)=>{
  ...
  attachementService.insertAttachment(docObject, file, true).then((data)=>{
    res.json(data);
  }, (err)=>{
    res.send(err);
  })
...
//By defualt attachments is saved with attached = false, if it's not attached to something for example project
//Then make attached = true if it assigned to something
router.post('/deattached',multipartMiddleware, (req, res)=>{

  const docObject = req.body;
  const name = docObject.name;
  const value = docObject.value;
  var file = req.files.file;
  attachementService.insertAttachment(docObject, file, false).then((data)=>{
    res.json(data);
  }, (err)=>{
    res.send(err);
  });
